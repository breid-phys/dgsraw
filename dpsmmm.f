C
      PROGRAM DPSMMM
C
C     Digisonde Portable Sounder (DPS) MMM data decoder
C
C     This program forms the basis of a decoder for Modified Maximum Method
C     (MMM) data obtained from a UMass Lowell DPS ionosonde.
C     This also includes .BEM files for multi-beam ionograms from the DPS.
...
C
C     Revision History
C     25Jul04  Initial version from dgsmmm, version 0.42
...
C      Variables
      ...
      INTEGER I,K,NA,IU,NBEAM,NDOPP,NCHAN,HINOISE
      INTEGER*1 IBUF(4096)
...
C     Filename must exist
      IF (K.LT.1) THEN
         WRITE(0,*) "dpsmmm-> ERROR: Input file must be specified"
         CALL EXIT(-1)
         RETURN
      ENDIF
C
...
C     Parse the command line.  Not very robust here, but it works. Mostly.
      DO NA = 1, K
        ...
        ELSE IF (CLP.EQ.'-h') THEN
          WRITE(*,*)' useage: dpsmmm {-vapsh} {-f #} filename'
          WRITE(*,*)' '
          WRITE(*,*)'	Decode MMM data from a DPS .MMM or .BEM file'
...
C
        ...
        ENDDO
C
C       Sorting and Noise Floor is for 16 Channel data
C       Sort the channels to change the output order
C16c      CALL SORT_SPECTRA(FRQ,APOL,HDOP,AZ,EL,5,  ICS)
C
C       Calculate the 'noise floor'.
C16c        FNOISE = AMPV(EAMPS,IC,NCHAN,MAXRBIN)
C
        IF (WRSPL) THEN

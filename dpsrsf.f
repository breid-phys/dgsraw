C
      PROGRAM DPSRSF
C
C     Digisonde Portable Sounder (DPS) RSF data decoder
C
C     This program forms the basis of a decoder for Routine Scientific Format
C     (RSF) data obtained from a UMass Lowell DPS ionosonde.
...
C     both their vintage and their source.  This code deals with only the RSF data
C
C     Terence Bullett
...
C
C     Useage:
C        dpsrsf inputfilename {options}
C        decoded data are written in ASCII to stdout
C
C        The input file must exist and contain RSF data from a DPS.
C
C     Revision History
...
C      Variables
      ...
      INTEGER I,K,NA,IU,NBEAM,NDOPP,NCHAN,HINOISE,IRXCONF
      INTEGER*1 IBUF(4096)
...
C     Filename must exist
      IF (K.LT.1) THEN
         WRITE(0,*) "dpsrsf-> ERROR: Input file must be specified"
         CALL EXIT(-1)
         RETURN
      ENDIF
C
...
C     Parse the command line.  Not very robust here, but it works. Mostly.
      DO NA = 1, K
        ...
        ELSE IF (CLP.EQ.'-h') THEN
          ...
          WRITE(*,*)' '
          WRITE(*,*)'	Decode RSF data from a DPS .RSF file'
          ...
          WRITE(*,*)'  -Z  <val>  Set the ARMENU ZENITH to <val>'
          WRITE(*,*)'  -R  <val>  Set the ARMENU RXCONFIG to <val>'
          WRITE(*,*)' '      

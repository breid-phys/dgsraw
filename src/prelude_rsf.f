...
C====+==================================================================+==
C
      INTEGER FUNCTION PRELUDE_RSF(IFRQ,PREFACE,IBUF,IPOL,FREQ,IGS)
C
...
C     07Jun10  TWB -Fixed polariztion bug
C
C     Requires:
C        UNP_BCD2 IRTYPE  FRQ_DPS P_BCD2
C
C====+==================================================================+==
C
...
C
C     Re-pack the BCD characters
      PREFACE(7) = P_BCD2(IMIN+MININC)
      PREFACE(8) = P_BCD2(ISEC)
      LSEC=ISEC
C     The MPA
      PRELUDE_RSF=UNP_BCD2(IBUF(IOFF+6))
C
C

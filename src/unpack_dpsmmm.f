...
C====+==================================================================+==
C
      LOGICAL FUNCTION UNPACK_DPSMMM(IBUF,   
     +            TIME,PREFACE,FREQ,FNOISE,EAMPS,EPHASE,IC,RGT,MAXRBIN,
     +            HINOISE)
C
C     This subroutine takes a 4K block of DPS MMM ionogram data and is built
C       on a similar routine for decoding 16 channel data.  There are some
C       features here unneeded for 16C data.
C
C     The DPS MMM format data provides the following output.
...
C
C     Revisions:
C     25Jul04 TWB - Initial version from UNPACK_MMM v0.42
C     28Jul04 TWB - Updated interface to PRELUDE() routine.
C     31Jul04 TWB - Removed the first 6 and last 12 range gates
C     12Jan08 TWB - Updated to PRELUDE_MMM()
C
C====+==================================================================+==
C
      ...
      INTEGER MAXRBIN,MAXCHAN, IRTYPE,IC(256),HINOISE
C
C     Local variables
      INTEGER I,IR,YYYY,ITMP,IFRQ,MAXFRQ,IPL,MPA,
     +        IG,IGS,IBT
      INTEGER*1 MASK,MASKL,MASKH
      REAL DBSCALE, DEG8
C
C     Functions
      INTEGER PRELUDE_MMM
...
C     Check for MMM data type.  Exit if not
      ...
      IF ((ITMP.NE.9).AND.(ITMP.NE.8)) THEN
         UNPACK_DPSMMM = .FALSE.
         WRITE(0,*) 'UNPACK_DPSMMM--> Not an MMM data block'
         RETURN
      ENDIF
...
C     is hardwired to the BlockType=1 data, because all of the
C     recorded DPS data are in this format.
...
C     This actually modifies the PREFACE() array, except for MPA
      MPA=PRELUDE_MMM(IFRQ,PREFACE,IBUF,IBT,FREQ,IGS)
      IF (MPA.EQ.-1) THEN
         UNPACK_DPSMMM=.FALSE.
         IFRQ = 0
         RETURN
...
C
C     Get the frequency value, for DPS remove the 2.5 kHz shift.
      FREQ = FRQ_D256(PREFACE) - 0.0025
C
C     The amplitude scale on the ionograms, from
C     4 upper bits give 6 dB per count
      DBSCALE = 6.0/16.0
C
      IGS = PREFACE(53)
      IG  = PREFACE(57)
...
cdb      write(*,*) dbscale,MPA,fnoise,preface(53),autogain
...
C     They are unsigned 8 bit integers so we have to treat them carefully
      DO IR=1,MAXRBIN
         DO I=1,MAXCHAN
            ...
            EPHASE(IR,IC(IR))=0.0
cdb            write(*,*) ir,iax,ic(ir),itmp,eamps(ir,ic(ir))
         ENDDO
      ENDDO
C

C     Remove the first 6 and last 12 range gates as being useless
      IF (.TRUE.) THEN
         MAXRBIN = MAXRBIN - 18
         DO IR=1,MAXRBIN
cdb            write(*,*) ir,eamps(ir,ic(ir)),ic(ir)
           DO I = 1,16
              EAMPS(IR,I)  = EAMPS(IR+6,I)
              EPHASE(IR,I) = EPHASE(IR+6,I)
           ENDDO
           RGT(IR) = RGT(IR+6)
           IC(IR) = IC(IR+6)
         ENDDO
